...
class DatabaseHelper {
  ...
  Future<void> _onCreate(Database db, int version) async {
    await db.execute('CREATE TABLE Goals(id INTEGER PRIMARY KEY AUTOINCREMENT, daily_calories INTEGER NOT NULL, carb_percentage INTEGER NOT NULL, protein_percentage INTEGER NOT NULL, fat_percentage INTEGER NOT NULL, sugar_percentage INTEGER NOT NULL, auto_calorie_mode INTEGER NOT NULL DEFAULT 0, custom_percent_per_month REAL NOT NULL DEFAULT 1.0, use_custom_start_calories INTEGER NOT NULL DEFAULT 0, user_start_calories INTEGER NOT NULL DEFAULT 2000, user_age INTEGER NOT NULL DEFAULT 30, user_activity_level REAL NOT NULL DEFAULT 1.3, last_monday_check TEXT, first_week_initialized INTEGER NOT NULL DEFAULT 0, user_height REAL NOT NULL DEFAULT 170)');
    await db.execute('CREATE TABLE ConsumedFoods(id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, meal_name TEXT NOT NULL, food_id INTEGER NOT NULL, quantity INTEGER NOT NULL)');
    await db.execute('CREATE TABLE Settings(id INTEGER PRIMARY KEY AUTOINCREMENT, dark_mode INTEGER NOT NULL DEFAULT 0, reminder_weigh_enabled INTEGER NOT NULL DEFAULT 0, reminder_weigh_time TEXT NOT NULL DEFAULT \'08:00\', reminder_weigh_time2 TEXT NOT NULL DEFAULT \'09:00\', reminder_supplement_enabled INTEGER NOT NULL DEFAULT 0, reminder_supplement_time TEXT NOT NULL DEFAULT \'10:00\', reminder_supplement_time2 TEXT NOT NULL DEFAULT \'11:00\', reminder_meals_enabled INTEGER NOT NULL DEFAULT 0, reminder_breakfast TEXT NOT NULL DEFAULT \'07:00\', reminder_lunch TEXT NOT NULL DEFAULT \'12:30\', reminder_dinner TEXT NOT NULL DEFAULT \'19:00\')');
    await db.execute('CREATE TABLE WeightEntries(id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT NOT NULL, weight REAL NOT NULL)');
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 19) {
      try {
        await db.execute("ALTER TABLE Goals ADD COLUMN user_height REAL NOT NULL DEFAULT 170");
      } catch (_) {}
    }
  }
  ...
  Future<int> saveGoals({required int dailyCalories, required int carbPercentage, required int proteinPercentage, required int fatPercentage, required int sugarPercentage}) async {
    final db = await database;
  ...
  Future<List<ConsumedFoodItem>> getConsumedFoods(DateTime date) async {
    ...
    final List<Map<String, dynamic>> result = await db.query('ConsumedFoods', where: 'date = ?', whereArgs: [formattedDate]);
    List<ConsumedFoodItem> consumedFoods = [];
    for (var map in result) {
      final dummyFood = FoodItem(id: map['food_id'], name: '...', brand: '...', barcode: null, caloriesPer100g: 0, fatPer100g: 0.0, carbsPer100g: 0.0, sugarPer100g: 0.0, proteinPer100g: 0.0);
      consumedFoods.add(
        ConsumedFoodItem(
          id: map['id'],
          food: dummyFood,
  ...
  Future<void> saveDarkMode(bool isDarkMode) async {
    final db = await database;
    await db.update('Settings', {'dark_mode': isDarkMode ? 1 : 0}, where: 'id = 1');
  }
  ...
  Future<void> mergeData(String jsonData) async {
    ...
    if (data['consumed_foods'] is List) {
      for (var c in data['consumed_foods']) {
        List<Map<String, dynamic>> existingConsumed = await db.query('ConsumedFoods', where: 'date = ? AND meal_name = ? AND food_id = ?', whereArgs: [c['date'], c['meal_name'], c['food_id']]);
    ...
    if (data['weight_entries'] is List) {
      for (var w in data['weight_entries']) {
        List<Map<String, dynamic>> existingW = await db.query('WeightEntries', where: 'date = ? AND weight = ?', whereArgs: [w['date'], w['weight']]);
