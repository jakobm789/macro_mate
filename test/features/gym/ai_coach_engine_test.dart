...
void main() {
  group('AiCoachEngine 16-Sets & Lower-Body Tests', () {
    ...
    for (final days in [2, 3, 4, 5]) {
      test(
        'Split for $days days generates exactly 16 sets per routine with leg exercises',
        () {
          ...
          final routines =
              (plan['routines'] as List<dynamic>).cast<Map<String, dynamic>>();
          ...
          for (final routine in routines) {
            ...
            expect(
              ...
              equals(16),
              reason:
                  'Routine "${routine['name']}" in $days-day split must have exactly 16 sets',
            );
            ...
            for (final ex in exercises) {
              ...
              if (id == 'ex_squat' ||
                ...
                expect(
                  ...
                  equals(180),
                  reason:
                      'Compound exercise $id must have 180s (3 min) rest time',
                );
              }
            }
          }
    ...
    test(
        'buildIntakePrompt includes 16 sets, leg balance, and 180s rest instructions',
        () {
      final profile = GymIntakeProfile(
        goal: 'hypertrophy',
        experience: 'intermediate',
  ...
  group('GymController Rest-Time & Manual Plan Creation Tests', () {
    ...
    test(
      'default rest time is 180 seconds (3 minutes) and can be adjusted',
      () {
    ...
    test(
      'saveManualPlan creates and activates custom plan in database',
      () async {
        ...
        final exercises =
            controller.routineExercises[controller.routines.first.id];
    ...
    test(
      ...
      () async {
        ...
        expect(
          controller.activeSets.where((set) => set.exerciseId == 'ex_bench_press'),
          hasLength(1),
        );
    ...
    test(
      'deleteWorkoutSession deletes completed workout from database',
      () async {
        // 1. Finish a workout
        await controller.startWorkout(
          routine: GymPlanRoutineRow(
